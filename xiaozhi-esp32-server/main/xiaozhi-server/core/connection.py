...
class ConnectionHandler:
    def __init__(
        ...
        self.hardware_bridge = HardwareBridge(hardware_config)
        self.action_executor = ActionExecutor(self.hardware_bridge)
    ...
    async def _initialize_hardware(self):
        ...
        try:
            # 初始化硬件桥接
            await self.hardware_bridge.initialize()
            # 设置 WebSocket 连接（用于发送舵机指令）
            self.hardware_bridge.set_websocket(self.websocket)
            # 启动动作执行器
            await self.action_executor.start()
            self.logger.bind(tag=TAG).info("米特硬件控制系统初始化完成")
        except Exception as e:
            self.logger.bind(tag=TAG).warning(f"硬件控制系统初始化失败（非致命）: {e}")
    ...
    def chat(self, query, depth=0):
        ...
        # 米特动作系统：[ACT] 标记检测
        action_marker_detected = False
        action_buffer = ""
        pending_text = ""  # 用于缓存可能包含 [ACT] 的文本
        ...
        for response in llm_responses:
            ...
            if content is not None and len(content) > 0:
                # 如果已经检测到 [ACT]，只累积动作数据
                if action_marker_detected:
                    action_buffer += content
                    continue

                # 累积到 pending_text 用于检测 [ACT]
                pending_text += content

                # 检测 [ACT] 标记
                if "[ACT]" in pending_text:
                    action_marker_detected = True
                    # 分离文本和动作部分
                    parts = pending_text.split("[ACT]", 1)
                    text_part = parts[0]
                    action_buffer = parts[1] if len(parts) > 1 else ""

                    # 发送文本部分给 TTS
                    if text_part.strip() and not tool_call_flag:
                        response_message.append(text_part)
                        self.tts.tts_text_queue.put(
                            TTSMessageDTO(
                                sentence_id=self.sentence_id,
                                sentence_type=SentenceType.MIDDLE,
                                content_type=ContentType.TEXT,
                                content_detail=text_part,
                            )
                        )
                    pending_text = ""
                    continue

                # 检查是否可能是 [ACT] 的开始（避免截断）
                # 如果末尾有 [ 或 [A 或 [AC 或 [ACT，先缓存
                safe_to_send = pending_text
                for partial in ["[ACT]", "[ACT", "[AC", "[A", "["]:
                    if pending_text.endswith(partial):
                        safe_to_send = pending_text[:-len(partial)]
                ...
                # 在llm回复中获取情绪表情
                if emotion_flag and safe_to_send.strip():
                    asyncio.run_coroutine_threadsafe(
                        textUtils.get_emotion(self, safe_to_send),
                        self.loop,
                    )
                    emotion_flag = False

                # 流式发送给 TTS
                if safe_to_send and not tool_call_flag:
                    response_message.append(safe_to_send)
                    self.tts.tts_text_queue.put(
                        TTSMessageDTO(
                            sentence_id=self.sentence_id,
                            sentence_type=SentenceType.MIDDLE,
                            content_type=ContentType.TEXT,
                            content_detail=safe_to_send,
                        )
                    )

        # 处理剩余的 pending_text（如果没有检测到 [ACT]）
        if pending_text and not action_marker_detected and not tool_call_flag:
            response_message.append(pending_text)
            self.tts.tts_text_queue.put(
                TTSMessageDTO(
                    sentence_id=self.sentence_id,
                    sentence_type=SentenceType.MIDDLE,
                    content_type=ContentType.TEXT,
                    content_detail=pending_text,
                )
            )

        # 米特动作系统：解析动作 JSON
        if action_marker_detected and action_buffer.strip():
            action_cmd = self._parse_action_json(action_buffer)
            if action_cmd:
                self.logger.bind(tag=TAG).info(
                    f"解析动作指令: motion={action_cmd.motion.value}, "
                    f"emotion={action_cmd.emotion.value}, led={action_cmd.led}"
                )
                # 异步执行硬件动作
                asyncio.run_coroutine_threadsafe(
                    self.action_executor.execute_now(action_cmd),
                    self.loop,
                )
        # 处理function call
        if tool_call_flag:
    ...
    def _parse_action_json(self, json_str: str):
        """
        解析动作 JSON 字符串
        格式：{"motion": "nod", "emotion": "happy", "led": "rainbow"}
        """
        import re
        from core.hardware.action_protocol import ActionCommand, Motion, Emotion
        ...
        try:
            ...
            try:
                ...
                self.logger.bind(tag=TAG).debug(f"未知动作: {motion_str}")
            ...
            try:
                ...
                self.logger.bind(tag=TAG).debug(f"未知表情: {emotion_str}")
            ...
            self.logger.bind(tag=TAG).debug(f"解析动作 JSON 失败: {e}")
        return None
    ...
    async def close(self, ws=None):
        ...
        try:
            ...
            if hasattr(self, "action_executor") and self.action_executor:
                try:
                    await self.action_executor.stop()
                except Exception as e:
                    self.logger.bind(tag=TAG).debug(f"清理动作执行器时出错: {e}")
            if hasattr(self, "hardware_bridge") and self.hardware_bridge:
                try:
                    await self.hardware_bridge.close()
                except Exception as e:
                    self.logger.bind(tag=TAG).debug(f"清理硬件桥接时出错: {e}")
