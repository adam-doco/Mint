...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        self.connection_ready = False  # 连接是否已就绪
        self.connection_lock = asyncio.Lock()  # 连接锁
    ...
    async def open_audio_channels(self, conn):
        ...
        # 预先建立持久 WebSocket 连接
        await self._ensure_connection(conn)

    async def _ensure_connection(self, conn):
        """确保 WebSocket 连接已建立并就绪"""
        async with self.connection_lock:
            if self.asr_ws is not None and self.connection_ready:
                return True

            try:
                # 建立新的WebSocket连接
                headers = self.token_auth() if self.auth_method == "token" else None
                logger.bind(tag=TAG).info(f"正在预先建立ASR持久连接...")
                ...
                logger.bind(tag=TAG).info("ASR持久连接已建立并就绪")
                return True

            except Exception as e:
                logger.bind(tag=TAG).error(f"建立ASR持久连接失败: {str(e)}")
                if self.asr_ws:
    ...
    async def receive_audio(self, conn, audio, audio_have_voice):
        ...
        # 确保连接已建立（如果连接断开，尝试重新建立）
        if not self.connection_ready or self.asr_ws is None:
            await self._ensure_connection(conn)

        # 发送当前音频数据（只要连接就绪且有音频）
        if self.asr_ws and self.connection_ready and audio:
            try:
                ...
                logger.bind(tag=TAG).warning("ASR连接已断开，尝试重新连接...")
                self.connection_ready = False
                self.asr_ws = None
                await self._ensure_connection(conn)
            except Exception as e:
        ...
        try:
            while self.asr_ws and not conn.stop_event.is_set():
                ...
                try:
                    ...
                    logger.bind(tag=TAG).info("ASR服务连接已关闭，标记为需要重新连接")
                    self.connection_ready = False
                    self.asr_ws = None
                    break
                except Exception as e:
