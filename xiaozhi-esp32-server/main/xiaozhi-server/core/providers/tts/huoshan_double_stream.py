...
class TTSProvider(TTSProviderBase):
    ...
    async def text_to_speak(self, text, _):
        ...
        try:
            ...
            # 保存待处理的文本（TTS 2.0不在TTSSentenceStart中返回文本）
            self.pending_texts.append(filtered_text)
            ...
            while not self.conn.stop_event.is_set():
                try:
                    ...
                    if res.optional.event == EVENT_SessionCanceled:
                        ...
                        logger.bind(tag=TAG).info(f"TTSSentenceStart payload: {json_data}")
                        # 根据文档，文本在 res_params.text 字段中
                        res_params = json_data.get("res_params", {})
                        self.tts_text = res_params.get("text", "") or json_data.get("text", "")
